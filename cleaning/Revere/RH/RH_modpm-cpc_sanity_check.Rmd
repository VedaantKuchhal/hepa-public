...
*This file loads a dataframe created from running `RH_modpm-cpc_cleaning`. Make sure you run that file first (if you haven't already) before running this file.*

This file is used to sanity check data produced from `RH_modpm-cpc_cleaning`. Time-series plots are created of each sensor and given a quick visual check. If required, appropriate edits are to be made to `RH_modpm-cpc_cleaning`.
...
Participant IDs
```{r}
# Get relevant participant IDs from RH_participants
...
plot_types <- function(input_df, sp, y_c) {
  ...
  # PM 1 Number Concentration
  print(plot_one(input_df, sp, "pm1num", y_c))
  # PM 1
  print(plot_one(input_df, sp, "concent", y_c))
}
```
