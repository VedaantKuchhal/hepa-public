...
**HAVE YOU RUN `SS_modpm-cpc_cleaning`?**
*This file loads a dataframe created from running `SS_modpm-cpc_cleaning`. Make sure you run that file first (if you haven't already) before running this file.*

This file is used to sanity check data produced from `SS_modpm-cpc_cleaning`. Time-series plots are created of each sensor and given a quick visual check. If required, appropriate edits are to be made to `SS_modpm-cpc_cleaning`.
...
Participant IDs
```{r}
# Get relevant participant IDs from SS_participants
