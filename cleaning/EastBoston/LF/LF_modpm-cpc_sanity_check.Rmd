...
*This file loads a dataframe created from running `LF_modpm-cpc_cleaning`. Make sure you run that file first (if you haven't already) before running this file.*

This file is used to sanity check data produced from `LF_modpm-cpc_cleaning`. Time-series plots are created of each sensor and given a quick visual check. If required, appropriate edits are to be made to `LF_modpm-cpc_cleaning`.
...
Participant IDs
```{r}
# Get relevant participant IDs from LF_participants
