...
This file cleans, and calculates the I/O ratio for Mod-PM and CPC data. Output is saved in a `cleaned_modpm-cpc.RData` file that is *untracked* by Github. *All files that depend on using Mod-PM and/or CPC data must be run after running this file.*
...
```{r}
# Set path to data
path_to_data <- "data/EastBoston/LF/modpm-cpc/"
...
for (file_name in list.files(path_to_data)) {
  ...
  df <- df %>%
    ...
    select(date1min, HEPA_on, starts_with(c("bin", "pm", "concent"))) %>%
    # Filter out unnecessary columns (bin12;23, model_id, env, bin0_ratio)
    select(-contains(c(as.character(12:23), "model", "env", "bin0_", "MToF", "std"))) %>%
...
curated_df <- all_df %>% 
  ...
  # Rename date column and CPC count column to standard
  rename(date = date1min, 
         concent.indoor = concent_1.indoor, 
         concent.outdoor = concent_1.outdoor) %>%
...
### Save Data
The `main_df` dataframe is saved to `cleaned_modpm-cpc.RData`
```{r}
save(main_df, file = paste0("cleaning/EastBoston/LF/", "cleaned_modpm-cpc.RData"))
```
