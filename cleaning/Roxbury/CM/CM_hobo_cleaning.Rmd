...
time_map <- read_excel("data/Roxbury/CM/CM_mapping.xlsx") %>%
...
```{r}
# Set path to data
path_to_data <- "data/Roxbury/CM/hobo/"
...
for (file_name in list.files(path_to_data)) {
  ...
  df <- mutate(df, participant_id = apartment)
...
### Save Data
The `main_df` dataframe is saved to `cleaned_hobo.RData`
```{r}
save(main_df, file = paste0("cleaning/Roxbury/CM/", "cleaned_hobo.RData"))
```