...
**HAVE YOU RUN `SS_modpm_cleaning` AND `SS_modpm-cpc_cleaning`?**
*This file loads two dataframes created from running `SS_modpm_cleaning` and `SS_modpm-cpc_cleaning`. Make sure you run those files first (if you haven't already) before running this file.*
...
Results can be found in `summary/EastBoston/SS/s_SS_MC_quants.csv`.
...
write.csv(summary_normal, "summary/EastBoston/SS/s_SS_MC_quants.csv")
```
