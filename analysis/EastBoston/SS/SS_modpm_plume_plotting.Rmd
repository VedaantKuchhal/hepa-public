...
get_local_valleys <- function(data, peaks, min_threshold) {
  ...
  # Sorting peaks_df by peak height in descending order
...
my_df_after %>%
  ...
  coord_cartesian(xlim = peak_idxs)
```
You may notice that the tallest peak in `decays_after` has a lower decay
constant. Plotting the highest peaks, this trend does seem to continue here. It
is important to note that `decays_after` has very short peaks; the highest peak
has a height of 7.12 ug/m3, which does not deviate far from the baseline.
...
write.csv(plumes.data, "summary/EastBoston/SS/s_SS_M_allplumes.csv")
write.csv(plume_reduc.data, "summary/EastBoston/SS/s_SS_M_plumestats.csv")
```
