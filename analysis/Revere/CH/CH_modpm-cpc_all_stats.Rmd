# Revere Data Summarization - multiple (for Mod-PM)

## STOP

**HAVE YOU RUN `CH_modpm_cleaning` AND `CH_modpm-cpc_cleaning`?**
*This file loads two dataframes created from running `CH_modpm_cleaning` and `CH_modpm-cpc_cleaning`. Make sure you run those files first (if you haven't already) before running this file.*

This script is used to calculated summary statistics for multiple participants in the Revere pilot, for Mod-PM deployments by Olin.

Results can be found in `summary/Revere/CH/s_CH_MC_quants.csv`.
...
write.csv(summary_normal, "summary/Revere/CH/s_CH_MC_quants.csv")
```
