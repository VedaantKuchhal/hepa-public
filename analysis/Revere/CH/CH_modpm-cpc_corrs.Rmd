# Revere Correlation Calculations - multiple (for Mod-PM)

## STOP

**HAVE YOU RUN `CH_modpm-cpc_cleaning`?**
*This file loads the dataframe created from running `CH_modpm-cpc_cleaning`. Make sure you run that file first (if you haven't already) before running this file.*

This script is used to calculate correlation coefficients for multiple participants in the Revere pilot, for Mod-PM and CPC deployments by Olin.

Results can be found in `summary/Revere/CH/s_CH_MC_corr.csv`.
...
```
## Save to file
Correlation coefficients are saved in a series of csv files. The naming convention of these files is specified by in `data_guide.md`
```{r}
# Save summary statistic files to CSVs
write.csv(corr_df, "summary/Revere/CH/s_CH_MC_corr.csv")
```
