...
**HAVE YOU RUN `CM_modpm_cleaning`?** *This file loads a dataframe
created from running `CM_modpm_cleaning`. Make sure you run that file
first (if you haven't already) before running this file.*

This file is used to analyze decay constants and complete plume
visualizations for the Roxbury Mod-PM data.
...
```{r}
# set path to data
path_to_data <- "cleaning/Roxbury/CM/"
```
...
curve_fitting <- function(data, peaks) {
  ...
  for (row in 1:nrow(peaks)) {
    row <- as.double(row)
    # If peak width is less than 20, skip!
    width <- peaks[row, 4] - peaks[row, 3]
    if (width < 20) {
      next
    }
...
```{r}
# Get data subset
my_df_subset <- my_df[my_df$participant_id=="300" & my_df$environment=="indoor",]
...
# Remove missing data
indices <- which(my_df_before$pm1 > 0)
beforepm25 <- my_df_before$pm10[indices]

indices <- which(my_df_after$pm1 > 0)
afterpm25 <- my_df_after$pm1[indices]
...
points(peaks_a[, 3], afterpm25[peaks_a[, 3]+1], pch = 20, col = "yellow")
points(peaks_a[, 4], afterpm25[peaks_a[, 4]], pch = 20, col = "green")
```
...
```{r}
get_plumes <- function(sub_data, id, pm) {
  # Separate sub data by before vs. after
  my_df_before <- sub_data[sub_data$case=='before',]
  my_df_after  <- sub_data[sub_data$case=='after',]
  
  # Remove missing data
  indices <- which(my_df_before[[pm]] > 0)
  beforepm <- my_df_before[[pm]][indices]
  
  indices <- which(my_df_after[[pm]] > 0)
  afterpm <- my_df_after[[pm]][indices]
  ...
  # Get peaks from before and after
  peaks_b <- findpeaks(beforepm,
                       nups = 1,
                       ndowns = 2,
                       minpeakheight = as.numeric(pm_min_heights[pm_min_heights$case=='before', 'mean']),
                       minpeakdistance = 200,
                       threshold = 0)
  peaks_a <- findpeaks(afterpm,
                       nups = 1,
                       ndowns = 2,
                       minpeakheight = as.numeric(pm_min_heights[pm_min_heights$case=='after', 'mean']),
                       minpeakdistance = 200,
  ...
  # Get valleys before and after to isolate each plume
  peaks_b <- get_local_valleys(beforepm,
                               peaks = peaks_b,
                               min_threshold = as.numeric(pm_baseline[pm_baseline$case=='before', 'mean']),
                               min_length = 10)
  peaks_a <- get_local_valleys(afterpm,
                               peaks = peaks_a,
                               min_threshold = as.numeric(pm_baseline[pm_baseline$case=='after', 'mean']),
                               min_length = 10)
  
  # perform decay analysis for before and after
  decays_before <- curve_fitting(beforepm, peaks = peaks_b)
  decays_after  <- curve_fitting(afterpm, peaks = peaks_a)
...
for (id in unique(my_df$participant_id)) {
  for (pm in c("pm1", "pm25", "pm10")) {
    print(paste("Calculating PM", pm, "for participant", id, "in environment", "indoor"))
    # Get relevant data
    sub_data <- my_df[my_df$participant_id == id & my_df$environment == "indoor",]
    if (length(sub_data$pm25) < 200) next
    
    new_plumes <- get_plumes(sub_data, id, pm)
...
Save 
```{r}
write.csv(plumes.data, file = paste0("summary/Roxbury/CM/", "s_CM_M_allplumes.csv"))
write.csv(plume_reduc.data, file = paste0("summary/Roxbury/CM/", "s_CM_M_plumestats.csv"))
```
